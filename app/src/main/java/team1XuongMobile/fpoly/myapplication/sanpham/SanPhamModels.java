package team1XuongMobile.fpoly.myapplication.sanpham;

public class SanPhamModels {
    private String idSanPham;
    private String tenSp;
    private String maSp;
    private String khoiLuong;
    private String giaNhap;
    private String giaBan;
    private String thueAvailable;
    private String thueDauRa;
    private String thueDauVao;
    private String trangThaiAvailable;
    private String ten_loai;
    private String id_loai;
    private String id_nha_cc;
    private String ten_nha_cc;
    private String uid;
    private String codeIme;
    private String timestamp;
    private String img;

    private String kh;

    public SanPhamModels(String idSanPham, String tenSp, String maSp, String khoiLuong, String giaNhap, String giaBan, String thueAvailable, String thueDauRa, String thueDauVao, String trangThaiAvailable, String ten_loai, String id_loai, String id_nha_cc, String ten_nha_cc, String uid, String codeIme, String timestamp, String img, String kh) {
        this.idSanPham = idSanPham;
        this.tenSp = tenSp;
        this.maSp = maSp;
        this.khoiLuong = khoiLuong;
        this.giaNhap = giaNhap;
        this.giaBan = giaBan;
        this.thueAvailable = thueAvailable;
        this.thueDauRa = thueDauRa;
        this.thueDauVao = thueDauVao;
        this.trangThaiAvailable = trangThaiAvailable;
        this.ten_loai = ten_loai;
        this.id_loai = id_loai;
        this.id_nha_cc = id_nha_cc;
        this.ten_nha_cc = ten_nha_cc;
        this.uid = uid;
        this.codeIme = codeIme;
        this.timestamp = timestamp;
        this.img = img;
        this.kh = kh;
    }



    public SanPhamModels() {
    }

    public String getKh() {
        return kh;
    }

    public void setKh(String kh) {
        this.kh = kh;
    }

    public String getIdSanPham() {
        return idSanPham;
    }

    public void setIdSanPham(String idSanPham) {
        this.idSanPham = idSanPham;
    }

    public String getTenSp() {
        return tenSp;
    }

    public void setTenSp(String tenSp) {
        this.tenSp = tenSp;
    }

    public String getMaSp() {
        return maSp;
    }

    public void setMaSp(String maSp) {
        this.maSp = maSp;
    }

    public String getKhoiLuong() {
        return khoiLuong;
    }

    public void setKhoiLuong(String khoiLuong) {
        this.khoiLuong = khoiLuong;
    }

    public String getGiaNhap() {
        return giaNhap;
    }

    public void setGiaNhap(String giaNhap) {
        this.giaNhap = giaNhap;
    }

    public String getGiaBan() {
        return giaBan;
    }

    public void setGiaBan(String giaBan) {
        this.giaBan = giaBan;
    }

    public String getThueAvailable() {
        return thueAvailable;
    }

    public void setThueAvailable(String thueAvailable) {
        this.thueAvailable = thueAvailable;
    }

    public String getThueDauRa() {
        return thueDauRa;
    }

    public void setThueDauRa(String thueDauRa) {
        this.thueDauRa = thueDauRa;
    }

    public String getThueDauVao() {
        return thueDauVao;
    }

    public void setThueDauVao(String thueDauVao) {
        this.thueDauVao = thueDauVao;
    }

    public String getTrangThaiAvailable() {
        return trangThaiAvailable;
    }

    public void setTrangThaiAvailable(String trangThaiAvailable) {
        this.trangThaiAvailable = trangThaiAvailable;
    }

    public String getTen_loai() {
        return ten_loai;
    }

    public void setTen_loai(String ten_loai) {
        this.ten_loai = ten_loai;
    }

    public String getId_loai() {
        return id_loai;
    }

    public void setId_loai(String id_loai) {
        this.id_loai = id_loai;
    }

    public String getId_nha_cc() {
        return id_nha_cc;
    }

    public void setId_nha_cc(String id_nha_cc) {
        this.id_nha_cc = id_nha_cc;
    }

    public String getTen_nha_cc() {
        return ten_nha_cc;
    }

    public void setTen_nha_cc(String ten_nha_cc) {
        this.ten_nha_cc = ten_nha_cc;
    }

    public String getUid() {
        return uid;
    }

    public void setUid(String uid) {
        this.uid = uid;
    }

    public String getCodeIme() {
        return codeIme;
    }

    public void setCodeIme(String codeIme) {
        this.codeIme = codeIme;
    }

    public String getTimestamp() {
        return timestamp;
    }

    public void setTimestamp(String timestamp) {
        this.timestamp = timestamp;
    }

    public String getImg() {
        return img;
    }

    public void setImg(String img) {
        this.img = img;
    }
}
